...
export async function getApplicantApplications(applicantId: string) {
  ...
  const applicantApplicationsQuery = () =>
    supabase
      .from("applications")
      .select(
        "*, inspections(scheduled_at,status,plumbing_approved,remarks,inspected_at), payments(id,payment_type,amount,due_date,office_payment_at,status,paid_at,official_receipt_number,notes), concessionaires(*)"
      )
      .eq("applicant_id", applicantId)
  ...
  const applicantApplicationsLegacyQuery = () =>
    supabase
      .from("applications")
      .select(
        "*, inspections(scheduled_at,status,plumbing_approved,remarks,inspected_at), payments(id,payment_type,amount,due_date,status,paid_at,official_receipt_number,notes), concessionaires(*)"
      )
      .eq("applicant_id", applicantId)
      .order("created_at", { ascending: false });
